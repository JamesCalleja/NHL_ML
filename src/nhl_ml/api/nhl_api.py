...
class NHLStats:
    ...
    def get_team_roster(self, team_abbrev: str) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"Found {len(all_players)} players on {team_abbrev} roster")
            return all_players
    ...
    def get_player_stats(self, player_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            if stats_data:
                logger.info(f"Found stats for {stats_data.get('firstName', {}).get('default', '')} {stats_data.get('lastName', {}).get('default', '')}")
                return stats_data
    ...
    def get_all_team_stats(self) -> Dict[str, List[Dict]]:
        ...
        for team_abbrev in self.teams:
            ...
            logger.info(f"Processed {len(team_stats)} players for {team_abbrev}")
